...
const Switch = React.forwardRef<HTMLInputElement, SwitchProps>(
  (
    ...
    // Init from controlled prop first, then defaultChecked, then false.
    ...
    // Keep local state in sync when the controlled prop changes externally.
    React.useEffect(() => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Always update local state immediately so the animation plays at once.
    ...
    return (
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundColor: isChecked ? '#2845D6' : 'var(--color-border)',
            borderColor:      isChecked ? '#2845D6' : 'var(--color-border)',
          }}
        >
          {/* Handle container */}
          <div
          ...
          >
            ...
            <div
              className={cn(
                ...
                isChecked ? 'bg-white' : 'bg-[var(--color-text-primary)]',
                isPressed
                  ? handleSizePressed
              ...
              style={{ color: isChecked ? '#2845D6' : 'var(--color-text-muted)' }}
            >
              {shouldRenderIcons && (
            ...
            <div
              className={cn(
                ...
                isChecked
                  ? 'left-[14px]'
                  : shouldRenderIcons && !isSmall ? 'left-[14px]' : 'left-[10px]',
                isSmall && (isChecked ? 'left-[10px]' : 'left-[8px]'),
              )}
              style={{ backgroundColor: isChecked ? '#2845D6' : 'var(--color-text-primary)' }}
            />
          </div>
        </div>
      </label>
