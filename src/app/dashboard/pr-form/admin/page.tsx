    ...
    >
      ...
      >
        ...
        <div className="px-6 pb-2 max-h-[calc(100vh-22rem)] overflow-y-auto [scrollbar-width:thin]">
          ...
          {item.prf_type === 'emergency_loan' && item.emergency_loan && (() => {
            const el = item.emergency_loan;
            const [yyyy, mm, dd] = el.starting_date.split('-').map(Number);
            const fmtDt = new Date(yyyy, mm - 1, dd).toLocaleDateString('en-US', {
            ...
            return (
              <div className="mt-4 pt-4">
                ...
                <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                  <DetailField label="Loan Amount"           value={fmtAmt(el.amount)} />
                  <DetailField label="Number of Cut-Offs"    value={`${el.number_of_cutoff} cut-off${el.number_of_cutoff > 1 ? 's' : ''}`} />
                  <DetailField label="Starting Date"         value={fmtDt} />
                  <DetailField label="Deduction per Cut-Off" value={fmtAmt(el.deduction_per_cutoff)} />
                </div>
              </div>
            );
          })()}
          ...
          {item.prf_type === 'medicine_allowance' && item.medicine_allowance && (() => {
            const ma = item.medicine_allowance;
            const fmtAmt = (v: string | number) =>
              '₱' + Number(v).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
            ...
            return (
              <div className="mt-4 pt-4">
                ...
                <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                  <DetailField label="Period Start"      value={fmtMonthYear(ma.start_date)} />
                  <DetailField label="Period End"        value={fmtMonthYear(ma.end_date)} />
                  <DetailField label="Requested Amount"  value={fmtAmt(ma.amount)} />
                </div>
              </div>
            );
          })()}
