...
const PRF_TYPES_BY_CATEGORY: Record<string, { value: string; label: string; unavailable?: boolean }[]> = {
  ...
  hr_payroll: [
    ...
    { value: 'medicine_allowance',      label: 'Medicine Allowance', unavailable: true },
...
function EmergencyLoanModal({
  ...
  const isValid            = amount !== null && cutoff !== null && !!startDate && nameValid && !fullNameErr;
        ...
        <div className="space-y-4 p-6 max-h-[calc(100vh-14rem)] overflow-y-auto [scrollbar-width:thin]">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              ...
              onValueChange={v => setCutoff(parseInt(v, 10))}
            ...
            <Popover open={dateOpen} onOpenChange={setDateOpen}>
              <PopoverTrigger asChild>
                <button
                  ...
                  className={cn(
                    ...
                    startDate ? 'text-[var(--color-text-primary)]' : 'text-[var(--color-text-muted)]',
                  )}
                >
                  {startDate ? fmtCalDate(startDate) : 'dd/mm/yyyy'}
                  <CalendarIcon size={14} className="text-[var(--color-text-muted)] shrink-0" />
                </button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0 z-[80]" align="start">
                <Calendar
                  ...
                  onSelect={d => { setStartDate(d ?? undefined); setDateOpen(false); }}
          ...
          <div className="space-y-1.5">
            <label className="text-[12px] font-semibold text-[var(--color-text-primary)] uppercase tracking-wide">
              Full Name Confirmation <span className="text-red-500 normal-case tracking-normal">*</span>
            </label>
            <Input
...
function SubmitPRFModal({
  ...
  async function handleSubmit() {
    ...
    // Emergency Loan goes through its own dedicated modal — do not POST here
...
interface PRFNotification {
  id: number;
  notification_type: string;
  title: string;
  message: string;
  is_read: boolean;
  related_prf_id: number | null;
  related_prf_control_number: string;
  created_at: string;
}
...
function NotificationModal({
  ...
  function typeColor(t: string) {
    if (t === 'prf_approved')    return 'text-emerald-600';
    if (t === 'prf_disapproved') return 'text-red-600';
    if (t === 'prf_cancelled')   return 'text-amber-600';
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-[60] flex items-start justify-end p-4 pt-16"
      onClick={onClose}
    >
      <motion.div
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-3">
          <div className="flex items-center gap-2">
            <Bell size={14} className="text-[var(--color-text-primary)]" />
            <span className="text-sm font-semibold text-[var(--color-text-primary)]">Notifications</span>
            {unreadCount > 0 && (
              <span className="inline-flex items-center justify-center h-4 min-w-4 px-1 rounded-full bg-[#2845D6] text-[10px] font-bold text-white">
          ...
          <div className="flex items-center gap-2">
            {unreadCount > 0 && (
              <button type="button" onClick={onMarkAllRead}
                className="text-[11px] text-[#2845D6] hover:underline transition-all">
                Mark all read
              </button>
            )}
            <button type="button" onClick={onClose}
              className="flex h-6 w-6 items-center justify-center rounded-full text-[var(--color-text-muted)] hover:bg-[var(--color-bg-card)] transition-colors">
              <X size={13} />
            </button>
          </div>
        </div>
            ...
            notifications.map(n => (
              <button
                ...
                className={cn(
                  ...
                  !n.is_read && 'bg-[#2845D6]/5',
                )}
              >
                <div className="flex items-start gap-2">
                  {!n.is_read && (
                    <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#2845D6] shrink-0" />
                  )}
                  <div className={cn('flex-1', n.is_read && 'pl-3.5')}>
                    <p className={cn('text-xs font-semibold', typeColor(n.notification_type))}>
                      {n.title}
                    </p>
                    <p className="text-xs text-[var(--color-text-primary)] mt-0.5 leading-relaxed">
                      {n.message}
                    </p>
                    <p className="text-[11px] text-[var(--color-text-muted)] mt-1">{fmtAgo(n.created_at)}</p>
                  </div>
                </div>
              </button>
            ))
...
export default function PRFormPage() {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      const res = await fetch('/api/activitylog/notifications/', { credentials: 'include' });
      if (!res.ok) return;
      const data: PRFNotification[] = await res.json();
  ...
  useEffect(() => {
    ...
    fetchNotifications().then(() => { prevUnreadRef.current = notifications.filter(n => !n.is_read).length; });
    const id = setInterval(fetchNotifications, 30_000);
    return () => clearInterval(id);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [authLoaded, fetchNotifications]);

  async function markAllRead() {
    try {
      await fetch('/api/activitylog/notifications/read-all/', {
  ...
  async function markRead(id: number) {
    try {
      await fetch(`/api/activitylog/notifications/${id}/read/`, {
        method: 'POST', credentials: 'include',
        headers: { 'X-CSRFToken': getCsrfToken() },
      });
            ...
            >
              ...
              {notifications.filter(n => !n.is_read).length > 0 && (
                <span className="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center
                  rounded-full bg-[#2845D6] px-0.5 text-[9px] font-bold text-white">
